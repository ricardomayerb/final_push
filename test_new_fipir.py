...
x_names_z = ['C', 'I', 'K', 'L', 'Y', 'zetaa']
x_names_ckl_az = ['a', 'C', 'K', 'L', 'zetaa']
x_names_ckl_z = ['C', 'K', 'L', 'zetaa']
...
xw_sym_dicts_z = fipir_new.set_x_w_sym_dicts(x_names_z, w_names)
x_s_d_z, w_s_d_z = xw_sym_dicts_z

xw_sym_dicts_ckl_az = fipir_new.set_x_w_sym_dicts(x_names_ckl_az, w_names)
x_s_d_ckl_az, w_s_d_ckl_az = xw_sym_dicts_ckl_az

xw_sym_dicts_ckl_z = fipir_new.set_x_w_sym_dicts(x_names_ckl_z, w_names)
x_s_d_ckl_z, w_s_d_ckl_z = xw_sym_dicts_ckl_z
...
x_in_ss_sym_d_z = {st: x_s_d_z[st] for st in x_s_d_z.keys() if 'ss' in st}
x_in_ss_sym_li_z = x_in_ss_sym_d_z.values()
x_in_ss_sym_d_ckl_az = {st: x_s_d_ckl_az[st]
                        for st in x_s_d_ckl_az.keys() if 'ss' in st}
x_in_ss_sym_li_ckl_az = x_in_ss_sym_d_ckl_az.values()
x_in_ss_sym_d_ckl_z = {st: x_s_d_ckl_z[st]
                       for st in x_s_d_ckl_z.keys() if 'ss' in st}
x_in_ss_sym_li_ckl_z = x_in_ss_sym_d_ckl_z.values()
...
# x_in_ss_sym_li_z
# Out[26]: [C_ss, I_ss, K_ss, zetaa_ss, L_ss, Y_ss]
#
# x_in_ss_sym_li
...
xini_z[4] = xini_az[5]
xini_z[5] = xini_az[6]
xss_ini_dict_z = dict(zip(x_in_ss_sym_li_z, xini_z))
# x_in_ss_sym_li_ckl_az
# Out[24]: [zetaa_ss, K_ss, a_ss, C_ss, L_ss]
# x_in_ss_sym_li
# Out[27]: [a_ss, C_ss, I_ss, K_ss, zetaa_ss, L_ss, Y_ss]
xini_ckl_az = np.zeros((5))
xini_ckl_az[0] = xini_az[4]
xini_ckl_az[1] = xini_az[3]
xini_ckl_az[2] = xini_az[0]
xini_ckl_az[3] = xini_az[1]
xini_ckl_az[4] = xini_az[5]
xss_ini_dict_ckl_az = dict(zip(x_in_ss_sym_li_ckl_az, xini_ckl_az))
# x_in_ss_sym_li_ckl_az
# Out[24]: [zetaa_ss, K_ss, a_ss, C_ss, L_ss]
#
# x_in_ss_sym_li_ckl_z
# Out[25]: [zetaa_ss, K_ss, C_ss, L_ss]
xini_ckl_z = np.zeros((5))
xini_ckl_z[0] = xini_ckl_az[0]
xini_ckl_z[1] = xini_ckl_az[1]
xini_ckl_z[2] = xini_ckl_az[3]
xini_ckl_z[3] = xini_ckl_az[4]
xss_ini_dict_ckl_z = dict(zip(x_in_ss_sym_li_ckl_z, xini_ckl_z))
...
invout_dict = {Yt: sympy.exp((1-alpha)*at) * Kt**(alpha) * Lt**(1-alpha),
               Ytp1: sympy.exp((1-alpha)*atp1) * Ktp1**(alpha) * Ltp1**(1-alpha),
               It: sympy.exp(at) * Ktp1 - (1-delta)*Kt}

glist_cikly_az = eq_conditions_toy_FI_CIKLY_az()
glist_cikly_z = [x.subs(az_dict) for x in glist_cikly_az]
glist_cikly_z = [x for x in glist_cikly_z if x != 0]
glist_ckl_az = [x.subs(invout_dict) for x in glist_cikly_az]
glist_ckl_az = [x for x in glist_ckl_az if x != 0]
glist_ckl_z = [x.subs(az_dict) for x in glist_ckl_az]
glist_ckl_z = [x for x in glist_ckl_z if x != 0]
...
my_glist_cikly_z = [x.subs(az_dict) for x in my_glist_cikly_az]
my_glist_cikly_z = [x for x in my_glist_cikly_z if x != 0]
my_glist_ckl_az = [x.subs(my_invout_dict) for x in my_glist_cikly_az]
my_glist_ckl_az = [x for x in my_glist_ckl_az if x != 0]
my_glist_ckl_z = [x.subs(az_dict) for x in my_glist_ckl_az]
my_glist_ckl_z = [x for x in my_glist_ckl_z if x != 0]


# elw_fi_novals = fipir_new.FullInfoModel(ss_elw, x_names, w_names, param_names,
#                                 eq_conditions=glist_cikly_az)

# elw_fi_vals_noxini = fipir_new.FullInfoModel(ss_elw, x_names, w_names, param_names,
# eq_conditions=glist_cikly_az, par_to_values_dict=all_param_values_dict)
